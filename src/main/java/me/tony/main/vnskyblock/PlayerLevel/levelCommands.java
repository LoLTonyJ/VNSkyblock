...
public class levelCommands implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        ...
        if (permCheck.isAdmin(p)) {
            if (args.length == 0) {
                p.sendMessage(ChatColor.format("&bUsage: /la add/set <player> <num>"));
            }
            if (args.length == 2) {
                String subCommand = args[0];
                if (subCommand.equalsIgnoreCase("get")) {
                    Player target = Bukkit.getPlayer(args[1]);
                    p.sendMessage(ChatColor.format(target.getDisplayName() + " &cLevel is &a" + playerManager.getLevel(target)));
                }
